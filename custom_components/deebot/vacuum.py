...
class EcovacsVacuum(VacuumDevice):
    ...
    def fan_speed_list(self):
        ...
        return [ozmo.FAN_SPEED_NORMAL, ozmo.FAN_SPEED_HIGH]

    def turn_on(self, **kwargs):
        """Turn the vacuum on and start cleaning."""

        self.device.run(ozmo.Clean())
    ...
    def stop(self, **kwargs):
        ...
        self.device.run(ozmo.Stop())

    def clean_spot(self, **kwargs):
        """Perform a spot clean-up."""

        self.device.run(ozmo.Spot())

    def locate(self, **kwargs):
        """Locate the vacuum cleaner."""

        self.device.run(ozmo.PlaySound())

    def set_fan_speed(self, fan_speed, **kwargs):
        """Set fan speed."""
        if self.is_on:

            self.device.run(ozmo.Clean(mode=self.device.clean_status, speed=fan_speed))

    def send_command(self, command, params=None, **kwargs):
        """Send a command to a vacuum cleaner."""
        self.device.run(ozmo.VacBotCommand(command, params))
